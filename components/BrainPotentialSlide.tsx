...
const BrainPotentialSlide: React.FC<BrainPotentialSlideProps> = ({ data }) => {
  ...
  // Save score to unified context when game ends
  useEffect(() => {
    if (gameState === GameState.Summary && shuffledChallenges.length > 0) {
      const total = shuffledChallenges.length;
      const percentage = (score / total) * 100;
      updateModuleScore('module2', score, total, percentage >= 50); // Consider 50%+ as completed
    }
  }, [gameState, score, shuffledChallenges.length, updateModuleScore]);
  ...
  return (
    <div className="min-h-screen bg-gray-900 font-sans flex flex-col items-center p-4 relative overflow-hidden w-full h-full">
        ...
        <footer className="w-full max-w-4xl z-10 text-xs text-gray-500 flex justify-between items-center py-2 shrink-0">
            ...
            <span className="font-display tracking-wider">POWERED BY AI MODELS</span>
        </footer>
    </div>
  );
};

export default BrainPotentialSlide;
