      ...
      {(gameState === GameState.GAME_OVER || gameState === GameState.VICTORY) && (
        <div className="bg-white/90 backdrop-blur-md p-8 rounded-2xl shadow-2xl text-center max-w-md mx-4 animate-fade-in">
          <h2 className={`text-4xl font-bold mb-4 ${gameState === GameState.VICTORY ? 'text-green-600' : 'text-red-600'}`}>
            {gameState === GameState.VICTORY ? t('aiBalloon.missionAccomplished') : t('aiBalloon.gameOver')}
          </h2>
          <p className="text-gray-700 text-xl mb-6">
            {t('aiBalloon.finalScore')} <span className="font-bold text-blue-600">{finalScore}</span>
          </p>
          <div className="space-y-4">
            <p className="text-sm text-gray-500">
              {gameState === GameState.VICTORY
                ? t('aiBalloon.victoryMessage')
                : t('aiBalloon.defeatMessage')}
            </p>
