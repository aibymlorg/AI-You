...
import type { Challenge } from './brain-potential-types';
import { Target, ChallengeCategory } from './brain-potential-types';
...
const GameScreen: React.FC<GameScreenProps> = ({ challenge, onAnswer, progress, hintUsed, isHintVisible, onUseHint }) => {
  ...
  const getCategoryText = (category: ChallengeCategory) => {
    switch(category) {
      case ChallengeCategory.Fact: return 'Fact';
      case ChallengeCategory.UseCase: return 'Use Case';
      default: return '';
    }
  };
    ...
    <div className="flex flex-col items-center w-full">
      <div className="w-full max-w-2xl mb-2">
        <div className="flex justify-between items-center mb-2">
            <h2 className="font-display text-cyan-400">Question {progress.current} / {progress.total}</h2>
      ...
      <div className="h-8 mb-2 flex items-center justify-center">
        {isHintVisible && (
          <div className="bg-gray-700/50 text-yellow-300 px-4 py-1 rounded-full text-sm font-medium">
            Hint: This is a "{getCategoryText(challenge.category)}"
          </div>
        )}
      </div>
