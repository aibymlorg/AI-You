...
import { NavBar } from './future-of-ai/NavBar';
import { ReportView } from './future-of-ai/ReportView';
import { QuizGame } from './future-of-ai/QuizGame';
import { SlideData } from '../types';

interface FutureOfAISlideProps {
  data: SlideData;
}

const FutureOfAISlide: React.FC<FutureOfAISlideProps> = ({ data }) => {
    const [appState, setAppState] = useState<FutureAIAppState>(FutureAIAppState.HOME);
    ...
    return (
        <div className="relative min-h-screen overflow-x-hidden bg-gradient-to-br from-brand-bg to-secondary-dark text-primary-light font-inter antialiased selection:bg-primary-dark selection:text-white">
            ...
            <main className="relative z-10 p-4 sm:p-6 md:p-8 max-w-6xl mx-auto min-h-screen flex flex-col">
                ...
                <header className={`text-center transition-all duration-500 ${appState === FutureAIAppState.HOME ? 'py-20 md:py-32' : 'py-8'}`}>
                    <h1
                        className={`font-outfit font-bold tracking-tight transition-all duration-500 cursor-pointer ${
                            appState === FutureAIAppState.HOME ? 'text-4xl sm:text-5xl md:text-7xl' : 'text-2xl md:text-3xl'
                        }`}
                        onClick={() => setAppState(FutureAIAppState.HOME)}
                    >
                        The <span className="text-background">Future of AI</span>
                    </h1>

                    {appState === FutureAIAppState.HOME && (
                        <div className="animate-fade-in" style={{ animationDelay: '0.1s' }}>
                            ...
                            <div className="mt-10 flex flex-col sm:flex-row justify-center gap-4">
                                <button
                                    onClick={() => setAppState(FutureAIAppState.LEARN)}
                                ...
                                <button
                                    onClick={() => setAppState(FutureAIAppState.QUIZ)}
                ...
                {appState !== FutureAIAppState.HOME && (
                    <>
                        ...
                        <div className="flex-grow pb-12">
                            {appState === FutureAIAppState.LEARN && <ReportView />}
                            {appState === FutureAIAppState.QUIZ && <QuizGame />}
                        </div>
                    </>
                )}
            </main>
...
export default FutureOfAISlide;
