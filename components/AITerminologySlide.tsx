...
import { AITermAppView } from '../ai-terminology-types';
import LearningDeck from './ai-terminology/LearningDeck';
import QuizChallenge from './ai-terminology/QuizChallenge';
import AITutor from './ai-terminology/AITutor';
import { TERMS } from '../ai-terminology-constants';
import { BrainCircuit, Play, BookOpen, RotateCcw, Award, Shield } from 'lucide-react';
import { SlideData } from '../types';
import { useScore } from '../contexts/ScoreContext';

interface AITerminologySlideProps {
  data: SlideData;
}

const AITerminologySlide: React.FC<AITerminologySlideProps> = ({ data }) => {
  const { updateModuleScore } = useScore();
  const [view, setView] = useState<AITermAppView>('home');
  ...
  // Save score to unified context when quiz is completed
  useEffect(() => {
    if (view === 'victory' && score > 0) {
      const percentage = (score / maxScore) * 100;
      updateModuleScore('module3', score, maxScore, percentage >= 50); // Consider 50%+ as completed
    }
  }, [view, score, maxScore, updateModuleScore]);
        ...
        {view === 'home' && (
          <div className="text-center max-w-2xl animate-in zoom-in duration-500">
            ...
            <h1 className="text-5xl md:text-7xl font-display font-black text-white mb-6 leading-tight">
              Master the <br/>
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-violet-400">Current State of AI</span>
            </h1>
...
export default AITerminologySlide;
